                ...
                if "events" in self.kwargs:
                    for trigger in list(self.kwargs["events"]):
                        if type(trigger) != bytes:
                            ...
                            self.kwargs["events"][trigger] = self.kwargs["events"][
                                original
                            ]
                            del self.kwargs["events"][original]
                        if type(self.kwargs["events"][trigger]) != bytes:
                            self.kwargs["events"][trigger] = bytes(
                                self.kwargs["events"][trigger], "UTF-8"
                            )

                        if trigger.lower() in self.trace_log[last_trigger_pos:].lower():
                            trigger_pos = (
                                self.trace_log[last_trigger_pos:]
                                .lower()
                                .find(trigger.lower())
                            )
                    ...
                    if len(self.kwargs["events"]) == 0:
                        if "debug" in self.kwargs and self.kwargs["debug"]:
                            log(
                                f"Waiting for last command {self.cmd[0]} to finish.",
                                origin="spawn",
                                level=4,
                            )
                        ...
                        if (
                            bytes(f"]$".lower(), "UTF-8")
                            in self.trace_log[0 - len(f"]$") - 5 :].lower()
                        ):
                            if "debug" in self.kwargs and self.kwargs["debug"]:
