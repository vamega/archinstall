    ...
    for root, folders, files in os.walk(base):
        for file in files:
            ...
            if os.path.splitext(file)[1] == ".py":
                if len(
                    mac := re.findall("(([a-zA-z0-9]{2}[-:]){5}([a-zA-z0-9]{2}))", file)
                ):
...
class Profile:
    ...
    def path(self, *args, **kwargs):
        ...
        for path in [
            "./profiles",
            "/etc/archinstall",
            "/etc/archinstall/profiles",
            os.path.abspath(f"{os.path.dirname(__file__)}/../profiles"),
        ]:  # Step out of /lib
    ...
    def load_instructions(self, namespace=None):
        if (absolute_path := self.path) :
            if os.path.splitext(absolute_path)[1] == ".py":
                ...
                raise ProfileError(
                    f"Extension {os.path.splitext(absolute_path)[1]} is not a supported profile model. Only .py is supported."
                )

        raise ProfileError(
            f"No such profile ({self._path}) was found either locally or in {UPSTREAM_URL}"
        )
...
class Application(Profile):
    ...
    def path(self, *args, **kwargs):
        ...
        for path in [
            ...
            "/etc/archinstall/applications",
            "/etc/archinstall/profiles/applications",
            os.path.abspath(f"{os.path.dirname(__file__)}/../profiles/applications"),
        ]:
            if os.path.isfile(f"{path}/{self._path}.py"):
                return os.path.abspath(f"{path}/{self._path}.py")
        ...
        try:
            if (
                cache := grab_url_data(f"{UPSTREAM_URL}/applications/{self._path}.py")
            ) :
                self._cache = cache
