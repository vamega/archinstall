...
class Installer:
    ...
    def __init__(
        ...
        boot_partition,
        *,
        base_packages="base base-devel linux linux-firmware efibootmgr nano",
        profile=None,
        mountpoint="/mnt",
        hostname="ArchInstalled",
    ):
    ...
    def pacstrap(self, *packages, **kwargs):
        ...
        if (sync_mirrors := sys_command("/usr/bin/pacman -Syy")).exit_code == 0:
            if (
                pacstrap := sys_command(
                    f'/usr/bin/pacstrap {self.mountpoint} {" ".join(packages)}',
                    **kwargs,
                )
            ).exit_code == 0:
                return True
    ...
    def set_mirrors(self, mirrors):
        return use_mirrors(
            mirrors, destination=f"{self.mountpoint}/etc/pacman.d/mirrorlist"
        )

    def genfstab(self, flags="-Pu"):
        o = b"".join(
            sys_command(
                f"/usr/bin/genfstab -pU {self.mountpoint} >> {self.mountpoint}/etc/fstab"
            )
        )
    ...
    def set_locale(self, locale, encoding="UTF-8", *args, **kwargs):
        ...
        return (
            True
            if sys_command(
                f"/usr/bin/arch-chroot {self.mountpoint} locale-gen"
            ).exit_code
            == 0
            else False
        )
    ...
    def set_timezone(self, zone, *args, **kwargs):
        ...
        o = b"".join(
            sys_command(
                f"/usr/bin/arch-chroot {self.mountpoint} ln -s /usr/share/zoneinfo/{zone} /etc/localtime"
            )
        )
        return True
    ...
    def add_bootloader(self):
        ...
        o = b"".join(
            sys_command(
                f"/usr/bin/arch-chroot {self.mountpoint} bootctl --no-variables --path=/boot install"
            )
        )
            ...
            if self.partition.encrypted:
                for root, folders, uids in os.walk("/dev/disk/by-uuid"):
                    for uid in uids:
                        ...
                        if not os.path.basename(real_path) == os.path.basename(
                            self.partition.real_device
                        ):
                            continue

                        entry.write(
                            f"options cryptdevice=UUID={uid}:luksdev root=/dev/mapper/luksdev rw intel_pstate=no_hwp\n"
                        )
                ...
                for root, folders, uids in os.walk("/dev/disk/by-partuuid"):
                    for uid in uids:
                        ...
                        entry.write(
                            f"options root=PARTUUID={uid} rw intel_pstate=no_hwp\n"
                        )
    ...
    def user_create(self, user: str, password=None, groups=[], sudo=False):
        ...
        o = b"".join(
            sys_command(
                f"/usr/bin/arch-chroot {self.mountpoint} useradd -m -G wheel {user}"
            )
        )
        if password:
        ...
        if groups:
            for group in groups:
                o = b"".join(
                    sys_command(
                        f"/usr/bin/arch-chroot {self.mountpoint} gpasswd -a {user} {group}"
                    )
                )
    ...
    def user_set_pw(self, user, password):
        ...
        o = b"".join(
            sys_command(
                f"/usr/bin/arch-chroot {self.mountpoint} sh -c \"echo '{user}:{password}' | chpasswd\""
            )
        )
        pass
