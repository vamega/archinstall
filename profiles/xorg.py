...
AVAILABLE_DRIVERS = {
    ...
    "AMD / ATI": {"amd": ["xf86-video-amdgpu"], "ati": ["xf86-video-ati"]},
    "intel": ["xf86-video-intel"],
    "nvidia": {"open source": ["xf86-video-nouveau"], "proprietary": ["nvidia"]},
    "mesa": ["mesa"],
...
def select_driver(options):
    ...
    if len(drivers) >= 1:
        ...
        if (
            selected_driver.isdigit()
            and (pos := int(selected_driver)) <= len(drivers) - 1
        ):
        ...
        if type(selected_driver) == dict:
            ...
            selected_driver_package_group = input(
                f"Which driver-type do you want for {initial_option}: "
            )
            if (
                selected_driver_package_group.isdigit()
                and (pos := int(selected_driver_package_group))
                <= len(driver_options) - 1
            ):
                selected_driver_package_group = selected_driver[driver_options[pos]]
            elif selected_driver_package_group in selected_driver:
                selected_driver_package_group = selected_driver[
