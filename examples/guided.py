...
def perform_installation(device, boot_partition, language, mirrors):
    ...
    with archinstall.Installer(
        device, boot_partition=boot_partition, hostname=hostname
    ) as installation:
        ## if len(mirrors):
...
while (
    disk_password := getpass.getpass(
        prompt="Enter disk encryption password (leave blank for no encryption): "
    )
) :
    disk_password_verification = getpass.getpass(
...
while (
    root_pw := getpass.getpass(
        prompt="Enter root password (leave blank to leave root disabled): "
    )
) :
    root_pw_verification = getpass.getpass(
        prompt="And one more time for verification: "
    )
...
while 1:
    ...
    if len(new_user.strip()) == 0:
        if len(root_pw.strip()) == 0:
            archinstall.log(
                " * Since root is disabled, you need to create a least one (super) user!",
                bg="black",
                fg="red",
            )
            continue
    ...
    new_user_passwd_verify = getpass.getpass(
        prompt=f"Enter password again for verification: "
    )
...
while 1:
    ...
    if type(profile) != str:  # Got a imported profile
        ...
        profile = profile[
            0
        ]._path  # Once the prep is done, replace the selected profile with the profile name ("path") given from select_profile()
        break
    else:
        break
...
with archinstall.Filesystem(harddrive, archinstall.GPT) as fs:
    ...
    if disk_password:
        ...
        with archinstall.luks2(
            harddrive.partition[1], "luksloop", disk_password
        ) as unlocked_device:
